...
const ICONS = {
  ...
  Quizzes: (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      ...
      <path d="M6 6.5c0-1.1.9-2 2-2s2 .9 2 2c0 1-1 1.5-2 2" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
  ...
  Exercises: (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      ...
      <path d="M5 7l2 2 4-3" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  Students: (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      ...
      <path d="M11 7.5a2 2 0 0 0 0-4M15 13c0-2.2-1.79-4-4-4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
    </svg>
  ),
  Classes: (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
  ...
  Settings: (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      ...
      <path d="M8 1v1.5M8 13.5V15M1 8h1.5M13.5 8H15M2.93 2.93l1.06 1.06M12.01 12.01l1.06 1.06M2.93 13.07l1.06-1.06M12.01 3.99l1.06-1.06" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
    </svg>
  ),
  Curriculum: (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <rect x="2" y="1" width="9" height="11" rx="1.5" stroke="currentColor" strokeWidth="1.4"/>
      <path d="M5 4h3M5 7h3M5 10h1.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
      <rect x="5" y="4" width="9" height="11" rx="1.5" fill="white" stroke="currentColor" strokeWidth="1.4"/>
      <path d="M8 7h3M8 10h3M8 13h1.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
    </svg>
  ),
  Due: (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
...
// ─── Sidebar ──────────────────────────────────────────────────────────────────
export default function Navbar({ sidebarCollapsed = false, onToggleSidebarCollapsed }) {
  const { user } = useAuth();
    ...
    >
      ...
      >
        ...
        >
          ...
          {!sidebarCollapsed && (
            <div className="leading-tight min-w-0">
              <div className="text-[14px] font-semibold text-gray-900 tracking-tight">Class OS</div>
              <div className={`text-[10px] font-medium uppercase tracking-widest ${isTeacher ? 'text-brand-500' : 'text-green-500'}`}>
        ...
        {onToggleSidebarCollapsed && (
          ...
          >
            <SidebarToggleIcon direction={sidebarCollapsed ? 'expand' : 'collapse'} />
          </motion.button>
        )}
      </div>

      {/* ── Divider ─────────────────────────────────────────── */}
      <div className={`mb-2 h-px bg-gray-100 ${sidebarCollapsed ? 'mx-2' : 'mx-3'}`} />
      ...
      <div className={`pb-4 space-y-0.5 ${sidebarCollapsed ? 'px-1.5' : 'px-2'}`}>
        <div className={`mb-2 h-px bg-gray-100 ${sidebarCollapsed ? 'mx-1' : 'mx-1'}`} />
          ...
          >
            ...
            {sidebarCollapsed ? (
              <div className="min-w-0 flex-1 text-left">
                <div className="truncate text-[11px] font-medium leading-tight text-gray-800">{displayName}</div>
                {displayEmail && (
                  <div className="truncate text-[10px] leading-tight text-gray-400">{displayEmail}</div>
                )}
              </div>
            ) : (
              <div className="min-w-0 flex-1">
                <div className="truncate text-[12px] font-medium text-gray-800">{displayName}</div>
                {displayEmail ? (
                  <div className="truncate text-[10px] text-gray-400">{displayEmail}</div>
                ) : (
                  <div className="truncate text-[10px] text-gray-400">No email on file</div>
                )}
              </div>
            )}
          </div>
...
// ─── SidebarLink ──────────────────────────────────────────────────────────────
function SidebarLink({ to, label, pathname, shouldReduceMotion, sidebarCollapsed, layoutSuffix = '' }) {
  ...
  return (
    <Link
      ...
      className={`relative flex items-center rounded-lg text-[13px] font-medium transition-colors group focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-600 focus-visible:outline-offset-2 ${
        ...
        isActive ? 'text-brand-700' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'
      }`}
    >
      ...
      <span className={`relative z-10 shrink-0 ${isActive ? 'text-brand-600' : 'text-gray-400 group-hover:text-gray-600'}`}>
        {icon}
      </span>
