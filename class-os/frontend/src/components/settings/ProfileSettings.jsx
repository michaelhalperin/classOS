...
import { deleteAccount, getCurrentUser, updateCurrentUser } from '../../api/auth.js';
...
function LogoutConfirmDialog({ open, onCancel, onConfirm }) {
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 id="logout-confirm-title" className="text-lg font-semibold text-gray-900">
          Log out?
        </h3>
...
function AccountSessionBanner({ profile }) {
  ...
  return (
    <>
      <div className="flex w-full flex-col gap-4 rounded-2xl border border-gray-200/90 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:gap-6 md:p-6">
        <div className="flex min-w-0 items-start gap-4">
          ...
          <div className="min-w-0 pt-0.5">
            <p className="truncate text-lg font-semibold text-gray-900">{name || 'Account'}</p>
            {email ? <p className="truncate text-sm text-gray-500">{email}</p> : null}
            {role ? (
      ...
      <LogoutConfirmDialog open={logoutOpen} onCancel={closeLogoutDialog} onConfirm={confirmLogout} />
    </>
  );
}
...
function AccountDangerZone({ role }) {
  ...
  return (
    ...
    >
      <h2 id="danger-zone-heading" className="text-base font-semibold text-red-950 md:text-lg">
        Danger zone
      </h2>
      <p className="mt-2 text-sm leading-relaxed text-red-900/85">{blurb}</p>
      <div className="mt-5 max-w-md space-y-3">
        <div>
          <label htmlFor="delete-account-password" className="label text-gray-900">
        ...
        {deleteMut.isError ? (
          <p className="text-sm text-red-700" role="alert">
            {deleteMut.error?.response?.data?.message || 'Could not delete account.'}
          </p>
        ) : null}
      </div>
    </section>
...
export default function ProfileSettings({ description, aside = null }) {
  ...
  if (isError) {
    const msg =
      error?.response?.data?.message || error?.message || 'Could not load your profile.';
    return (
  ...
  const mainColumn = (
    <>
      ...
      {description ? (
        <p className="text-sm leading-relaxed text-gray-600 md:text-[0.9375rem]">{description}</p>
      ) : null}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        >
          <div className="min-h-0 w-full space-y-4 rounded-2xl border border-gray-200/90 bg-white p-5 shadow-sm md:p-6">
            <h2 className="text-base font-semibold text-gray-900 md:text-lg">Profile</h2>
            <div>
          ...
          <div className="min-h-0 w-full space-y-4 rounded-2xl border border-gray-200/90 bg-white p-5 shadow-sm md:p-6">
            <h2 className="text-base font-semibold text-gray-900 md:text-lg">Security</h2>
            ...
            <div>
              ...
              <input
                {...register('currentPassword', {
                  validate: (v) =>
                    !watch('newPassword') || (v && v.length > 0) || 'Required to change password',
                })}
                type="password"
                className="input"
                autoComplete="current-password"
            ...
            <div>
              ...
              <input
                {...register('confirmPassword', {
                  validate: (v) => !newPwd || v === newPwd || 'Passwords do not match',
                })}
                type="password"
                className="input"
                autoComplete="new-password"
        ...
        {mutation.isError && (
          <div className="rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700 w-full">
            {mutation.error?.response?.data?.message || 'Could not save changes.'}
          </div>
        )}
