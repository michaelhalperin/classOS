  ...
  return (
    ...
    >
        ...
        >
          ...
          >
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1">
              {ins.staleStudents.length}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              No login in {ins.staleLoginDays}+ days
            </p>
          </motion.div>
          <motion.div
            variants={shouldReduce ? undefined : cardVariants}
          ...
          >
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1">
              {ins.behindStudents.length}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              At least one missing submission
            </p>
          </motion.div>
          <motion.div
            variants={shouldReduce ? undefined : cardVariants}
          ...
          >
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1">
              {ins.dueThisWeek.length}
            </p>
            <p className="text-xs text-gray-500 mt-1">
              Assignments due Mon–Sun
            </p>
          </motion.div>
        </motion.div>
      ...
      <section>
        ...
        >
          <motion.div
            variants={shouldReduce ? undefined : analyticsBlockVariants}
            className="card p-4"
          >
          ...
          >
            ...
            <p className="text-xs text-gray-500 mb-2">
              Hours after the due time (negative = early). Only graded work with
              a due date.
            </p>
            <dl className="grid grid-cols-2 gap-2 text-sm">
              <div>
                ...
                <dd className="font-semibold text-gray-900">
                  {ins.submissionLatencyHours?.avg != null
                    ? `${ins.submissionLatencyHours.avg.toFixed(1)} h`
                    : "—"}
                </dd>
              </div>
              <div>
          ...
          >
            <h3 className="text-sm font-semibold text-gray-800 mb-3">
              Assignment difficulty (missing rate)
            </h3>
                  ...
                  >
                    {ins.assignmentDifficulty.slice(0, 12).map((row) => (
                      ...
                      >
                        ...
                        <td className="py-2 pr-4 text-gray-500 truncate max-w-[160px]">
                          {row.lessonTitle || "—"}
                        </td>
                        <td className="py-2">
                          {Math.round(row.missingRate * 100)}% (
                          {row.classSize - row.submittedCount}/{row.classSize})
                        </td>
                      </motion.tr>
                    ))}
                  </motion.tbody>
          ...
          >
            <h3 className="text-sm font-semibold text-gray-800 mb-3">
              Time on lesson (tracked)
            </h3>
            <p className="text-xs text-gray-500 mb-2">
              Sum of active time students spent on each lesson (30s heartbeats
              while the tab is open).
            </p>
            {ins.lessonTime.length === 0 ? (
              <p className="text-sm text-gray-500">
                No data yet — students generate this by reading lessons.
              </p>
            ) : (
              <motion.ul
                variants={shouldReduce ? undefined : sectionVariants}
