...
import { getQuizzes, getQuizAttempts, submitQuizAttempt } from '../../api/quizzes.js';
    ...
    return (
      <PageLayout fullWidth>
        <div className="card mx-auto max-w-lg px-4 pt-16 pb-24 text-center sm:pb-28 md:pt-20 md:pb-32">
          ...
          <p className="text-sm text-gray-500">Ask your teacher to enroll you.</p>
        </div>
      </PageLayout>
    );
  }
        ...
        <div className="pb-24 sm:pb-28 md:pb-32">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-gray-100 pb-4 md:mb-6 md:pb-5">
            <div className="min-w-0">
              <h1 className="text-xl font-bold tracking-tight text-gray-900 md:text-2xl">{activeQuiz.title}</h1>
              <p className="mt-1 text-sm text-gray-500">Answer all questions, then submit.</p>
            </div>
    ...
    <PageLayout fullWidth>
      {isLoading ? (
        ...
        >
          ...
          <h2 className="text-xl font-semibold text-gray-800">No quizzes yet</h2>
          <p className="mt-1 text-gray-500">Your teacher hasn&apos;t published any quizzes for this class.</p>
        </motion.div>
      ) : (
        <motion.div
        ...
        >
          <header className="border-b border-gray-100 pb-4 md:pb-5">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {quizzes.length} {quizzes.length === 1 ? 'quiz' : 'quizzes'} · start or review your attempts
            </p>
          </header>
    ...
    >
      <div className="flex flex-1 flex-col gap-4 sm:flex-row sm:items-start">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {done && (
              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                myAttempt.pct >= 80 ? 'bg-green-100 text-green-700' :
                myAttempt.pct >= 60 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
              }`}>
                {myAttempt.pct}%
              </span>
            )}
          ...
          {quiz.lessonId?.title && (
            <p className="text-xs text-brand-600 mt-0.5">{quiz.lessonId.title}</p>
          )}
    ...
    >
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between text-sm text-gray-500 mb-2">
          ...
          <span>{totalAnswered}/{questions.length} answered · {q.points} pt{q.points !== 1 ? 's' : ''}</span>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className={`card text-center py-8 border-2 ${
        pct >= 80 ? 'border-green-300 bg-green-50' :
        pct >= 60 ? 'border-yellow-300 bg-yellow-50' :
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <span className="text-xs text-gray-400 shrink-0">
                  {ans?.pointsEarned ?? 0}/{q.points ?? 1} pt{q.points !== 1 ? 's' : ''}
                </span>
              </div>
            </motion.div>
          );
