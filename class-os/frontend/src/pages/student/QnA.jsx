...
function QuestionCard({
  ...
  const pinMutation = useMutation({
    ...
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: ["questions", question.lessonId] }),
  });
  ...
  const isOwnQuestion =
    userId === (question.authorId?._id || question.authorId);

  return (
    <div
      className={`card transition-all ${question.isPinned ? "border-brand-200 bg-brand-50/30" : ""}`}
    >
      <div className="flex items-start gap-3">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1 text-xs text-gray-400">
            <span
              className={`font-medium ${question.authorId?.role === "teacher" ? "text-brand-600" : "text-gray-600"}`}
            >
      ...
      {expanded && (
        <div className="mt-4 pt-4 border-t border-gray-100">
          <p className="text-gray-700 text-sm whitespace-pre-wrap mb-5">
            {question.body}
          </p>
...
function AnswerItem({ answer, isTeacher, userId, questionId }) {
  ...
  return (
    <div
      className={`p-3 rounded-lg ${answer.authorId?.role === "teacher" ? "bg-brand-50 border border-brand-100" : "bg-gray-50"}`}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400 mt-1">
            <span
              className={`font-medium ${answer.authorId?.role === "teacher" ? "text-brand-600" : "text-gray-500"}`}
            >
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <textarea
            ...
            placeholder={
              isTeacher ? "Write your response…" : "Write your answer…"
            }
          />
          {errors.body && (
            <p className="mt-1 text-xs text-red-600">Enter a reply</p>
          )}
        </div>
        <button
          type="submit"
          className="btn-primary self-end py-2"
          disabled={mutation.isPending}
        >
