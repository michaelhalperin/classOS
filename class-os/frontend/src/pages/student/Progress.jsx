...
export default function StudentProgress() {
    ...
    return (
      <PageLayout fullWidth>
        <div className="card mx-auto max-w-lg px-4 pt-16 pb-24 text-center sm:pb-28 md:pt-20 md:pb-32">
          ...
          <h1 className="mb-2 text-xl font-semibold text-gray-800">My Progress</h1>
          <h2 className="mb-2 text-lg font-medium text-gray-700">Not enrolled in any class</h2>
          <p className="text-gray-500 text-sm">Ask your teacher to enroll you in a class.</p>
        </div>
      </PageLayout>
    );
  }
    ...
    return (
      <PageLayout fullWidth>
        <div className="card mx-auto max-w-lg px-4 pt-16 pb-24 text-center sm:pb-28 md:pt-20 md:pb-32">
          ...
          <p className="text-gray-500 text-sm">Use the class selector in the top bar.</p>
        </div>
      </PageLayout>
    );
  }
  ...
  const completedLessons = lessons.filter((l) => completedIds.includes(l._id)).length;
  ...
  const submittedCount = assignments.filter((a) => Boolean(a.submission?.submittedAt)).length;
  ...
  const pendingAssignments = assignments.filter(
    (a) => !a.submission?.submittedAt && a.dueDate && new Date(a.dueDate) >= new Date()
  );
  ...
  const gradeDistribution = [
    ...
    ...bucket,
    count: gradedSubs.filter((s) => s.grade >= bucket.min && s.grade <= bucket.max).length,
  }));
      ...
      {isLoading ? (
        ...
        >
          <header className="border-b border-gray-100 pb-4 md:pb-5">
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">My Progress</h1>
          ...
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            ...
            <StatCard
              ...
              color={avgGrade == null ? 'gray' : avgGrade >= 90 ? 'green' : avgGrade >= 70 ? 'yellow' : 'red'}
              shouldReduce={shouldReduce}
            />
            <StatCard
              label="Overdue"
              value={overdueAssignments.length}
              sub={overdueAssignments.length === 0 ? 'all caught up!' : 'need attention'}
          ...
          <div className="grid gap-3 md:gap-5 lg:grid-cols-12 lg:items-start">
            ...
            <div
              className={`flex flex-col gap-3 md:gap-5 ${
                hasGradeColumn ? 'lg:col-span-7 xl:col-span-8' : 'lg:col-span-12'
              }`}
            >
              <motion.div variants={shouldReduce ? undefined : itemVariants} className="card p-4 md:p-5">
                <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                  <h2 className="font-semibold text-gray-800">Lesson Progress</h2>
                  <Link to="/student/curriculum" className="text-sm font-medium text-brand-600 hover:underline">
                    Curriculum →
                  </Link>
                </div>
                <div className="mb-4 flex items-center gap-3">
                  <div className="h-3 flex-1 overflow-hidden rounded-full bg-gray-200">
                    <motion.div
                      ...
                      transition={{ ...spring, stiffness: 60, damping: 18, delay: 0.4 }}
                    />
                  </div>
                  <span className="w-10 text-right text-sm font-semibold text-brand-700">{lessonPct}%</span>
                </div>

                {lessons.length > 0 && (
                  <div className="space-y-2">
                    {[...new Set(lessons.map((l) => l.weekNumber))].sort((a, b) => a - b).map((week) => {
                      const wl = lessons.filter((l) => l.weekNumber === week);
                      const wDone = wl.filter((l) => completedIds.includes(l._id)).length;
                      const pct = Math.round((wDone / wl.length) * 100);
                      return (
                        <div key={week} className="flex items-center gap-2 text-sm sm:gap-3">
                          <span className="w-14 shrink-0 text-gray-500 sm:w-16">Week {week}</span>
                          <div className="h-2 flex-1 overflow-hidden rounded-full bg-gray-100">
                            <motion.div
                              className={`h-2 rounded-full ${pct === 100 ? 'bg-green-500' : 'bg-brand-400'}`}
                              initial={shouldReduce ? false : { width: 0 }}
                              animate={{ width: `${pct}%` }}
                              transition={{ ...spring, stiffness: 60, damping: 18, delay: 0.5 }}
                            />
                          </div>
              ...
              {assignmentTimeline.length > 0 && (
                <motion.div variants={shouldReduce ? undefined : itemVariants} className="card p-4 md:p-5">
                  <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <Link to="/student/homework" className="text-sm font-medium text-brand-600 hover:underline">
                      Homework →
                    </Link>
                  </div>
                  <div className="divide-y divide-gray-50">
                    {assignmentTimeline.map((a) => {
                      ...
                      const isOverdue = !isSubmitted && new Date(a.dueDate) < new Date();

                      return (
                        <div key={a._id} className="flex items-center gap-3 py-2.5 first:pt-0">
                          <div
                            className={`h-2.5 w-2.5 shrink-0 rounded-full ${
                              hasGrade ? 'bg-green-500' :
                              isSubmitted ? 'bg-yellow-400' :
                              isOverdue ? 'bg-red-400' : 'bg-gray-300'
                            }`}
                          />
                          ...
                          <div className="shrink-0 text-right">
                            {hasGrade ? (
                              <span className={`text-sm font-bold ${sub.grade >= 70 ? 'text-green-600' : 'text-red-500'}`}>
                                {sub.grade}
                  ...
                  {overdueAssignments.length > 0 && (
                    ...
                    >
                      <h3 className="mb-2 flex items-center gap-2 text-sm font-semibold text-red-600">
                        <span aria-hidden>⚠</span> Overdue ({overdueAssignments.length})
                      </h3>
                      ...
                      <Link to="/student/homework" className="mt-3 block text-sm font-medium text-red-600 hover:underline">
                        Submit now →
                      </Link>
                    </motion.div>
                  )}
                    ...
                    >
                      ...
                      <ul className="space-y-1.5">
                        {pendingAssignments.slice(0, 6).map((a) => (
                          <li key={a._id} className="flex items-center justify-between gap-2 text-sm text-gray-700">
            ...
            {hasGradeColumn && (
              <div className="flex flex-col gap-3 md:gap-5 lg:col-span-5 xl:col-span-4">
                <motion.div variants={shouldReduce ? undefined : itemVariants} className="card p-4 md:p-5">
                  <h2 className="mb-3 font-semibold text-gray-800">Grade distribution</h2>
                  <div className="space-y-2">
                    {gradeDistribution.map((bucket) => (
                      <div key={bucket.label} className="flex items-center gap-2 text-sm">
                        <span className="w-24 shrink-0 text-xs text-gray-500 sm:w-28">{bucket.label}</span>
                        <div className="h-4 min-h-[1rem] flex-1 overflow-hidden rounded-full bg-gray-100">
                          {bucket.count > 0 ? (
                            <motion.div
                              ...
                              transition={{ ...spring, stiffness: 60, damping: 18, delay: 0.5 }}
                            >
                              <span className="text-[10px] font-medium text-white">{bucket.count}</span>
                            </motion.div>
                          ) : null}
                        </div>
                      </div>
                ...
                <motion.div variants={shouldReduce ? undefined : itemVariants} className="card p-4 md:p-5">
                  <h2 className="mb-3 font-semibold text-gray-800">Graded work</h2>
                  <div className="divide-y divide-gray-50">
                    {gradedSubs
                      .sort((a, b) => new Date(b.submittedAt) - new Date(a.submittedAt))
                      .map((sub) => {
                        const assignmentTitle = sub.assignmentId?.title || 'Assignment';
                        return (
                          <div key={sub._id} className="flex items-center gap-3 py-2.5 first:pt-0">
                            <div
                              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-xs font-bold ${
                                sub.grade >= 90 ? 'bg-green-100 text-green-700' :
                            ...
                            <div className="min-w-0 flex-1">
                              <p className="truncate text-sm font-medium text-gray-800">{assignmentTitle}</p>
                              {sub.feedback && (
                                <p className="mt-0.5 truncate text-xs text-gray-500">{sub.feedback}</p>
                              )}
                            </div>
                            <span className="shrink-0 text-xs text-gray-400">
                  ...
                  <Link to="/student/homework" className="mt-3 block text-sm font-medium text-brand-600 hover:underline">
                    All homework →
                  </Link>
                </motion.div>
              </div>
          ...
          {lessons.length === 0 && assignments.length === 0 && (
            <motion.div variants={shouldReduce ? undefined : itemVariants} className="rounded-xl border border-dashed border-gray-200 bg-white/80 py-16 text-center">
              <p className="mb-2 text-4xl">📊</p>
              <p className="text-gray-500">No lessons or assignments yet for this class.</p>
            </motion.div>
          )}
        </motion.div>
      )}
