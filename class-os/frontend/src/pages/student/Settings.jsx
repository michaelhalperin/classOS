...
function CurriculumAside({ activeClassId, done, total, shouldReduce }) {
  ...
  if (!activeClassId) {
    return (
      <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50/80 p-5 md:p-6 lg:sticky lg:top-4">
        <p className="text-sm font-semibold text-gray-800">Curriculum progress</p>
        <p className="mt-2 text-sm leading-relaxed text-gray-600">
          Choose a class in the sidebar to see how many lessons you&apos;ve completed for that class.
        </p>
      </div>
    );
  }
  ...
  if (total === 0) {
    return (
      <div className="rounded-2xl border border-gray-200/90 bg-white p-5 shadow-sm md:p-6 lg:sticky lg:top-4">
        <p className="text-sm font-semibold text-gray-800">Curriculum progress</p>
        <p className="mt-2 text-sm leading-relaxed text-gray-600">
          There are no lessons in this class yet. When your teacher adds lessons, your progress will show here.
        </p>
        <Link
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200/90 bg-white shadow-sm lg:sticky lg:top-4">
      <div className="h-1 bg-gradient-to-r from-brand-500 via-brand-400 to-brand-600" aria-hidden />
      <div className="p-5 md:p-6">
        <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">This class</p>
        <p className="mt-2 text-sm font-semibold text-gray-900">Curriculum progress</p>
        <p className="mt-3 tabular-nums text-4xl font-bold text-gray-900">{pct}%</p>
        ...
        <div className="mt-4 h-2.5 w-full overflow-hidden rounded-full bg-gray-100">
          {pct > 0 && (
            <motion.div
              ...
              animate={{ width: `${pct}%` }}
              transition={{ ...spring, stiffness: 60, damping: 18, delay: 0.12 }}
            />
          )}
        </div>
        <Link
...
export default function StudentSettings() {
  ...
  const aside = (
    <CurriculumAside activeClassId={activeClassId} done={done} total={total} shouldReduce={shouldReduce} />
  );
      ...
      <div className="mx-auto w-full max-w-5xl pb-24 sm:pb-28 md:pb-32">
        <header className="border-b border-gray-100 pb-5 md:pb-6">
          ...
          <p className="mt-1.5 max-w-2xl text-sm leading-relaxed text-gray-600 md:text-base">
            Your name and sign-in details. Curriculum stats on the right reflect the class selected in the
            sidebar.
          </p>
        </header>
