...
export default function Assignments() {
  ...
  const saveMutation = useMutation({
    mutationFn: (data) =>
      editing ? updateAssignment(editing, data) : createAssignment(data),
    onSuccess: () => {
        ...
        <header className="flex flex-col gap-4 border-b border-gray-100 pb-5 md:flex-row md:items-center md:justify-between md:pb-6">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-sm text-gray-600">
              Link homework to lessons, set due dates, then grade submissions
        ...
        <AnimatePresence>
          {showForm && (
            <motion.div
              key="assignment-form"
              initial={
                shouldReduce ? { opacity: 0 } : { opacity: 0, height: 0 }
              }
              animate={
                shouldReduce ? { opacity: 1 } : { opacity: 1, height: "auto" }
              }
                    ...
                    <div className="flex gap-2 shrink-0">
                      ...
                      <button
                        onClick={() => {
                          if (
                            window.confirm(
                              "Delete this assignment and all its submissions?",
                            )
                          ) {
