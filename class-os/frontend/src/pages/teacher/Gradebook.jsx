...
export default function Gradebook() {
  ...
  function missingCount(studentId) {
    return assignments.filter((a) => getStatus(studentId, a._id) === "missing")
      .length;
  }
  ...
  const pendingGrades = submissions.filter(
    (s) => s.submittedAt && s.grade == null,
  ).length;
  const totalMissing = students.reduce(
    (sum, st) => sum + missingCount(st._id),
    0,
  );
      ...
      <PageLayout fullWidth>
        <div className="card mx-auto max-w-lg px-4 py-16 text-center">
          <p className="text-gray-700">
            Create a class first to use the gradebook.
          </p>
        </div>
      </PageLayout>
    );
        ...
        <header className="flex flex-col gap-2 border-b border-gray-100 pb-5 md:flex-row md:items-end md:justify-between md:pb-6">
          <div className="min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-sm text-gray-600">
              Cross-tab of students and assignments — click a cell to grade
              submitted work.
            </p>
          </div>
          {activeClassId ? (
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <SummaryCard
                ...
                value={pendingGrades}
                color={pendingGrades > 0 ? "yellow" : "green"}
              />
              <SummaryCard
                label="Total Missing"
                value={totalMissing}
              ...
              >
                ...
                <div className="px-5 py-3 border-t border-gray-100 flex flex-wrap gap-4 text-xs text-gray-500">
                  ...
                  <LegendItem
                    ...
                    label="Submitted (click to grade)"
                  />
                  <LegendItem
...
function GradeCell({ sub, status, onClick }) {
  const base =
    "inline-flex items-center justify-center w-10 h-8 rounded-lg border text-xs font-semibold cursor-default transition-colors";
  if (status === "graded") {
    return (
      <button
        onClick={onClick}
  ...
  return (
    <span
      title="Not submitted"
      className={`${base} bg-red-50 text-red-300 border-red-100`}
    >
      —
    </span>
  );
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="label">
              Grade <span className="text-gray-400 font-normal">(0 – 100)</span>
            </label>
            <input
              type="number"
              min={0}
