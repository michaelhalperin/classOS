...
function IconSparkle() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden="true">
      <path d="M6.5 1l1 3.5L11 5.5l-3.5 1L6.5 10l-1-3.5L2 5.5l3.5-1L6.5 1z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/>
    </svg>
  );
}
function IconPaperclip() {
  return (
    <svg width="13" height="13" viewBox="0 0 13 13" fill="none" aria-hidden="true">
      <path d="M11 5.5L5.5 11A3.182 3.182 0 011 6.5L6.5 1a2.121 2.121 0 013 3L4 9.5A1.06 1.06 0 012.5 8L8 2.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  );
}
function IconTrash() {
...
function IconSliders() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <path d="M2 4h12M5 4v2.5M11 4v5M2 10h12M8 10v4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
...
export default function LessonEditor() {
  ...
  const [addedSuggestedExerciseIndices, setAddedSuggestedExerciseIndices] = useState(() => new Set());
  /** Indices in `polishExtra.misconceptions` already quick-added. */
  const [addedMisconceptionIndices, setAddedMisconceptionIndices] = useState(() => new Set());
  ...
  const { register, handleSubmit, reset, watch, setValue, getValues, formState: { errors, isDirty } } = useForm({
    defaultValues: { title: "", content: "", objectives: "", misconceptionWarnings: "", weekNumber: 1, orderIndex: 1 },
  });
    ...
    onSuccess: async (saved) => {
      if (pendingFiles.length > 0) {
        ...
        for (const file of pendingFiles) {
          try { latest = await uploadAttachment(saved._id, file); } catch {}
        }
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      reset({ title: "", content: "", objectives: "", misconceptionWarnings: "", weekNumber: 1, orderIndex: 1 });
  ...
  const appendMisconceptionMutation = useMutation({
    mutationFn: async ({ m }) => {
      ...
      return updateLesson(selectedLesson, { ...lessonFormToPayload(v), misconceptionWarnings });
    },
  ...
  const runPolish = async (mode) => {
    ...
    try {
      ...
      if (mode === "expand") { setPolishExpand(data.markdown || ""); setPolishExtra(null); }
      else { setPolishExpand(""); setPolishExtra(data); }
    } catch (e) {
      alert(e.response?.data?.message || e.message);
    } finally {
      setPolishBusy(null);
        ...
        <div className="flex flex-col items-center justify-center h-full py-24 text-center">
          <div className="w-14 h-14 rounded-2xl bg-brand-50 border border-brand-100 flex items-center justify-center mb-4">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="#2147f5" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
          <h2 className="text-lg font-semibold text-gray-900 mb-1">No class selected</h2>
          <p className="text-sm text-gray-500 mb-6 max-w-xs">Create a class first — lessons belong to a class.</p>
      ...
      <div className="relative flex min-h-0 w-full flex-1">
        ...
        <aside
          className={`fixed inset-y-0 left-0 z-[30] flex w-[min(18.5rem,92vw)] shrink-0 flex-col border-r border-gray-200 bg-white transition-transform duration-200 ease-out lg:relative lg:z-auto lg:w-60 lg:max-w-none lg:translate-x-0 ${
            mobileLessonsOpen ? "max-lg:translate-x-0" : "max-lg:-translate-x-full"
          }`}
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-gray-100 px-4 py-4 max-md:pt-14">
            <span className="text-[13px] font-semibold text-gray-800">Lessons</span>
            <motion.button
              onClick={() => {
                handleNewLesson();
          ...
          <div className="flex-1 overflow-y-auto py-2">
              ...
              <div key={week} className="mb-1">
                ...
                {lessons
                  ...
                  .map((lesson) => {
                    ...
                    const isDragOver = dragOverId === lesson._id && dragId !== lesson._id;
                    return (
                      <div
                        key={lesson._id}
                        ...
                        onDragEnd={() => {
                          if (dragId && dragOverId && dragId !== dragOverId) {
                            const sorted = [...lessons].sort((a, b) =>
                              a.weekNumber !== b.weekNumber ? a.weekNumber - b.weekNumber : a.orderIndex - b.orderIndex
                            );
                            const fromIdx = sorted.findIndex((l) => l._id === dragId);
                            const toIdx = sorted.findIndex((l) => l._id === dragOverId);
                            const reordered = [...sorted];
                            const [moved] = reordered.splice(fromIdx, 1);
                            reordered.splice(toIdx, 0, moved);
                            const weekCounters = {};
                            const order = reordered.map((l) => {
                              weekCounters[l.weekNumber] = (weekCounters[l.weekNumber] || 0) + 1;
                              return { id: l._id, weekNumber: l.weekNumber, orderIndex: weekCounters[l.weekNumber] };
                            });
                        ...
                        onDragOver={(e) => { e.preventDefault(); setDragOverId(lesson._id); }}
                        onDragLeave={() => { if (dragOverId === lesson._id) setDragOverId(null); }}
                        onClick={() => {
                      ...
                      >
                        ...
                        {/* Order number badge */}
                        <span className={`shrink-0 text-[10px] font-mono ${isActive ? "text-brand-400" : "text-gray-300"}`}>
                          {lesson.orderIndex}
                        </span>
                      </div>
                    );
            ...
            {reorderMutation.isPending && (
              <p className="px-4 py-2 text-[11px] text-brand-500 animate-pulse">Saving order…</p>
            )}
          </div>
        </aside>
          ...
          >
            ...
            <div className="flex min-w-0 shrink-0 flex-nowrap items-center gap-2 border-b border-gray-200 bg-white px-3 py-2.5 max-md:pl-14 md:gap-3 md:px-6 md:py-3">
              <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
                ...
                <input
                  ...
                  className={`min-w-0 flex-1 truncate rounded-sm border-none bg-transparent text-base font-semibold text-gray-900 outline-none placeholder-gray-300 focus:ring-2 focus:ring-brand-500/30 md:text-[18px] ${
                    errors.title ? "ring-2 ring-red-400 ring-offset-1 ring-offset-white" : ""
                  }`}
                  placeholder="Untitled lesson…"
                  title={errors.title ? errors.title.message : undefined}
                  aria-invalid={errors.title ? "true" : undefined}
                />
                {errors.title && <span className="sr-only">{errors.title.message}</span>}
              </div>
              ...
              <div className="relative z-10 flex shrink-0 items-center gap-2 border-l border-gray-100 bg-white pl-2 md:gap-3 md:pl-3">
                ...
                <div className="flex shrink-0 items-center gap-0.5 rounded-lg bg-gray-100 p-0.5 md:gap-1 md:p-1">
                  <button
                    ...
                    className={`touch-manipulation flex items-center gap-1 rounded-md px-2 py-1 text-[11px] font-medium transition-all md:gap-1.5 md:px-2.5 md:text-[12px] ${
                      !preview ? "bg-white text-gray-800 shadow-sm" : "text-gray-500 hover:text-gray-700"
                    }`}
                  >
                ...
                <motion.button
                  type="submit"
                  disabled={saveMutation.isPending || (!isDirty && !!selectedLesson)}
                  whileHover={shouldReduce ? {} : { scale: 1.02 }}
                  whileTap={shouldReduce ? {} : { scale: 0.97 }}
                  transition={SNAPPY}
                  className="btn-primary touch-manipulation px-3 py-1.5 text-sm md:px-4"
                >
                  {saveMutation.isPending ? "Saving…" : selectedLesson ? "Save" : "Create"}
                </motion.button>
              </div>
            </div>
                  ...
                  >
                    ...
                    <div className="prose prose-sm max-w-none">
                      <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeSanitize]}>
                        {contentValue || "*No content yet — switch to Edit to write something.*"}
                      </ReactMarkdown>
                    </div>
                  </motion.div>
                ) : (
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-x-3 gap-y-1 border-t border-gray-100 bg-white px-3 py-2 text-[10px] text-gray-400 sm:text-[11px] md:gap-4 md:px-6">
              ...
              <span>{(contentValue || "").split(/\s+/).filter(Boolean).length} words</span>
              {isDirty && <span className="font-medium text-amber-500">● Unsaved</span>}
        ...
        >
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-4 py-4">
              ...
              >
                <PanelBlock className="shrink-0">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[11px] text-gray-500 mb-1 font-medium">Week</label>
                      <input
                        {...register("weekNumber", { required: true, valueAsNumber: true, min: 1 })}
                        type="number" min="1"
                        className="input text-sm py-1.5"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] text-gray-500 mb-1 font-medium">Order</label>
                      <input
                        {...register("orderIndex", { required: true, valueAsNumber: true, min: 1 })}
                        type="number" min="1"
                        className="input text-sm py-1.5"
                      />
                    </div>
                ...
                <PanelBlock className="flex min-h-0 flex-1 flex-col">
                  ...
                  <p className="text-[11px] text-gray-400 mb-2 shrink-0">Used by the AI tutor and quiz generator</p>
                  <textarea
                    {...register("objectives")}
                ...
                <PanelBlock className="shrink-0">
                  ...
                  <p className="text-[11px] text-gray-400 mb-2">Markdown shown to students (optional). Use AI Tools → quick add, or edit here.</p>
                  <textarea
                    {...register("misconceptionWarnings")}
                    rows={6}
                    className="input font-mono text-xs resize-y leading-relaxed"
                    placeholder={"#### Common slip\n\nWhy students confuse this…"}
                  />
                </PanelBlock>

                {!selectedLesson && (
                  <div className="shrink-0 rounded-xl bg-brand-50 border border-brand-100 p-3 text-[12px] text-brand-800">
                    <span className="font-semibold">Tip:</span> Save this lesson first to unlock AI tools and file uploads.
                  </div>
                )}
              </motion.div>
            )}
                ...
                {!selectedLesson ? (
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      {[
                        { mode: "expand", label: "Expand to fuller markdown", icon: "✦" },
                        { mode: "exercises", label: "Suggest exercises", icon: "⬡" },
                        { mode: "misconceptions", label: "Misconception warnings", icon: "⚠" },
                      ].map(({ mode, label, icon }) => (
                        <motion.button
                          key={mode}
                          type="button"
                    ...
                    <AnimatePresence>
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              setValue("content", cur ? `${cur}\n\n---\n\n${polishExpand}` : polishExpand, { shouldDirty: true });
                            }}
                          >
                            Append to content →
                          </button>
                        ...
                        >
                          ...
                          <ul className="space-y-2 max-h-64 overflow-y-auto">
                            {polishExtra.exercises.map((ex, i) => {
                              const alreadyAdded = addedSuggestedExerciseIndices.has(i);
                              const addingThis =
                                addSuggestedExerciseMutation.isPending &&
                                addSuggestedExerciseMutation.variables?.index === i;
                              return (
                              <li key={i} className="rounded-lg border border-gray-100 bg-gray-50 p-3 text-xs">
                                <p className="font-semibold text-gray-800">{ex.title}</p>
                                <p className="text-gray-600 mt-0.5 whitespace-pre-wrap">{ex.instructions}</p>
                                {ex.language && <span className="font-mono text-gray-400 text-[10px]">{ex.language}</span>}
                                <button
                                  type="button"
                                  disabled={
                                    alreadyAdded ||
                                  ...
                                  onClick={() => addSuggestedExerciseMutation.mutate({ ex, index: i })}
                                >
                                  {alreadyAdded ? "Added" : addingThis ? "Adding…" : "Add as exercise"}
                                </button>
                              </li>
                            );})}
                          </ul>
                        ...
                        >
                          ...
                          <ul className="space-y-2">
                            {polishExtra.misconceptions.map((m, i) => {
                              const warningAdded = addedMisconceptionIndices.has(i);
                              const addingThisWarning =
                                appendMisconceptionMutation.isPending &&
                                appendMisconceptionMutation.variables?.index === i;
                              return (
                              <li key={i} className="rounded-lg border border-amber-100 bg-amber-50/60 p-3 text-xs">
                                <p className="font-semibold text-gray-800">{m.title}</p>
                                <p className="text-gray-600 mt-0.5">{m.explanation}</p>
                                <p className="text-amber-700 mt-1 italic">{m.howToAddress}</p>
                                <button
                                  type="button"
                                  disabled={
                                    warningAdded ||
                                  ...
                                  onClick={() => appendMisconceptionMutation.mutate({ m, index: i })}
                                >
                                  {warningAdded ? "Added" : addingThisWarning ? "Adding…" : "Add to lesson"}
                                </button>
                              </li>
                            );})}
                          </ul>
                      ...
                      {insights && (
                        ...
                        >
                          ...
                          {insights.message && <p className="text-xs text-gray-500">{insights.message}</p>}
                          {insights.questionCount != null && (
                            <p className="text-xs text-gray-600 font-medium">{insights.questionCount} question(s) analysed</p>
                          )}
                          <ul className="space-y-2">
                            {(insights.clusters || []).map((c, i) => (
                              <li key={i} className="rounded-lg border border-gray-100 bg-white p-3 text-xs">
                                <span className={`text-[10px] uppercase font-semibold px-1.5 py-0.5 rounded ${
                                  c.severity === "high" ? "bg-red-100 text-red-800" :
                                  c.severity === "low"  ? "bg-gray-100 text-gray-600" :
            ...
            {rightPanelTab === "files" && (
              ...
              >
                {/* Upload button */}
                <label className={`flex items-center justify-center gap-2 w-full py-3 rounded-xl border-2 border-dashed text-[12.5px] font-medium cursor-pointer transition-colors ${
                  uploadingFile
                    ? "border-gray-200 text-gray-400 bg-gray-50 cursor-not-allowed"
                    : "border-brand-200 text-brand-600 bg-brand-50/50 hover:bg-brand-50 hover:border-brand-300"
                }`}>
                  {uploadingFile ? (
                    <><span className="w-4 h-4 border-2 border-brand-400 border-t-transparent rounded-full animate-spin" /> Uploading…</>
                  ) : (
                  ...
                  <div>
                    ...
                    <ul className="space-y-1.5">
                      {pendingFiles.map((file, idx) => (
                        <li key={idx} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-amber-50 border border-amber-100 text-xs">
                ...
                {attachments.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1.5">
                      {attachments.map((att) => (
                        <li key={att._id} className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-gray-50 border border-gray-100 text-xs">
