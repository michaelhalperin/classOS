...
export default function StudentDetail() {
  ...
  const mySubs = allSubs.filter(
    (s) => String(s.studentId?._id || s.studentId) === String(id),
  );
        ...
        <div className="card mb-6">
          ...
          <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <MiniStat
              ...
              color={
                avgGrade >= 70 ? "green" : avgGrade != null ? "red" : "gray"
              }
            />
          </div>
        </div>
          ...
          {assignments.map((a) => {
            ...
            return (
              <div key={a._id} className="flex items-center gap-3">
                ...
                <span
                  className={`shrink-0 text-xs font-medium px-3 py-1 rounded-full border ${colors[status]}`}
                >
                  {labels[status]}
                </span>
                {sub?.feedback && (
...
function SubmissionsTab({ subs, studentId, activeClassId }) {
  ...
  if (subs.length === 0) {
    return (
      <p className="text-gray-400 text-center py-10">No submissions yet.</p>
    );
  }
    ...
    <div className="card overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          {hasGrade ? (
            <span
              className={`font-bold ${sub.grade >= 70 ? "text-green-600" : "text-red-500"}`}
            >
              {sub.grade}
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 space-y-4">
              ...
              {sub.feedback && !grading && (
                <div>
                  ...
                  <p className="text-sm text-gray-700 italic bg-brand-50 rounded-lg p-3 border border-brand-100">
                    {sub.feedback}
                  </p>
                </div>
              )}
