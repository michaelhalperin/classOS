...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    const bodyContent =
      content != null && content !== "" ? content : existing?.draftContent;
    const bodyGh =
      githubLink != null
        ? String(githubLink).trim()
...
router.delete("/:id/retract", requireAuth, async (req, res) => {
  try {
    if (req.user.role !== "student") {
      return res
        .status(403)
        .json({ message: "Only students can retract submissions" });
    }
    ...
    if (!sub.submittedAt) {
      return res
        .status(400)
        .json({ message: "Submission has not been finalized yet." });
    }
    ...
    res.json({
      message: "Submission retracted. You can now edit and re-submit.",
      submission: sub,
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
