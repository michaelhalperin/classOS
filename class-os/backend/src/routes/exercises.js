  ...
  try {
    if (req.user.role === 'teacher') {
      ...
      if (lessonId) {
        ...
        const exercises = await Exercise.find({ lessonId }).populate('lessonId', 'title').sort({ createdAt: 1 });
        return res.json(exercises);
      }
      if (!classId || !mongoose.Types.ObjectId.isValid(classId)) {
        return res.status(400).json({ message: 'classId query parameter is required (or use lessonId)' });
      }
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const full = await Exercise.findById(req.params.id).populate('lessonId', 'title');
    res.json(full);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.post('/', requireAuth, requireTeacher, async (req, res) => {
  try {
    ...
    const exercise = await Exercise.create({ lessonId, title, instructions, starterCode, language });
