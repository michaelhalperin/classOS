...
router.post('/tutor', requireAuth, async (req, res) => {
  try {
    ...
    if (!lessonId || !Array.isArray(messages) || messages.length === 0) {
      return res.status(400).json({ message: 'lessonId and messages[] are required' });
    }
    ...
    const weekLessons = await Lesson.find({ weekNumber: lesson.weekNumber, classId: lesson.classId })
      .sort({ orderIndex: 1 })
    ...
    const reply = await tutorReply({ lesson, weekLessons, messages: sanitized });
    res.json({ reply });
  } catch (err) {
    console.error(err);
...
router.post('/code-hint', requireAuth, async (req, res) => {
  try {
    ...
    if (exerciseId && mongoose.isValidObjectId(exerciseId)) {
      const ex = await Exercise.findById(exerciseId).populate('lessonId', 'title content');
      if (ex) {
        exerciseTitle = ex.title;
        exerciseInstructions = ex.instructions || '';
        if (!lessonContent && ex.lessonId?.content) lessonContent = ex.lessonId.content;
        if (!lessonTitle && ex.lessonId?.title) lessonTitle = ex.lessonId.title;
      }
    }

    if (assignmentId && mongoose.isValidObjectId(assignmentId)) {
      const asg = await Assignment.findById(assignmentId).populate('lessonId', 'title content');
      if (asg) {
        assignmentTitle = asg.title;
        assignmentInstructions = asg.instructions || '';
        if (!lessonContent && asg.lessonId?.content) lessonContent = asg.lessonId.content;
        if (!lessonTitle && asg.lessonId?.title) lessonTitle = asg.lessonId.title;
      }
    }
...
router.post('/drills/generate', requireAuth, async (req, res) => {
  try {
    ...
    console.error(err);
    const status = err.status || 500;
    res.status(status).json({ message: err.message || 'Drill generation failed' });
  }
});
...
router.post('/drills/check', requireAuth, async (req, res) => {
  try {
    ...
    if (!drill || typeof userAnswer !== 'string') {
      return res.status(400).json({ message: 'drill object and userAnswer string are required' });
    }
    if (!drill.question || !drill.rubric || !drill.answerKey) {
      return res.status(400).json({ message: 'drill must include question, rubric, answerKey' });
    }
...
router.get('/insights/:lessonId', requireAuth, requireTeacher, async (req, res) => {
  try {
    if (!isAiConfigured()) return aiUnavailable(res);
    ...
    const questions = await Question.find({ lessonId }).sort({ createdAt: -1 }).select('_id title body');
    if (questions.length === 0) {
      return res.json({ clusters: [], message: 'No questions yet for this lesson.' });
    }
...
router.post('/polish', requireAuth, requireTeacher, async (req, res) => {
  try {
    ...
    if (!['expand', 'exercises', 'misconceptions'].includes(mode)) {
      return res.status(400).json({ message: 'mode must be expand, exercises, or misconceptions' });
    }
    const lesson = await Lesson.findById(lessonId);
    if (!lesson) return res.status(404).json({ message: 'Lesson not found' });
    const access = await assertLessonAccess(req, lesson);
...
router.post('/generate-quiz', requireAuth, requireTeacher, async (req, res) => {
  try {
    ...
    const quiz = await generateQuiz({ lesson, questionCount: Number(questionCount) || 8 });
    res.json(quiz);
  } catch (err) {
    console.error(err);
    res.status(err.status || 500).json({ message: err.message || 'Quiz generation failed' });
  }
});

// POST /ai/grade-submission — teacher: AI draft grade + feedback for a submission
router.post('/grade-submission', requireAuth, requireTeacher, async (req, res) => {
  try {
    if (!isAiConfigured()) return aiUnavailable(res);
    ...
    console.error(err);
    res.status(err.status || 500).json({ message: err.message || 'AI grading failed' });
  }
});
