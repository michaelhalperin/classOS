...
router.post('/run', requireAuth, async (req, res) => {
  try {
    ...
    if (!submitRes.ok) {
      ...
      return res.status(502).json({ message: `Judge0 submit failed: ${submitRes.status}`, detail: text });
    }

    const { token } = await submitRes.json();
    if (!token) return res.status(502).json({ message: 'No token returned from Judge0' });
    ...
    if (!result) return res.status(504).json({ message: 'Code execution timed out' });
