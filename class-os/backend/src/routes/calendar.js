...
async function assertClassAccess(classId, user) {
  ...
  const enrolled = cls.studentIds.some((id) => id.toString() === user._id.toString());
...
router.post('/:classId', requireAuth, requireTeacher, async (req, res) => {
  try {
    const cls = await Classroom.findOne({ _id: req.params.classId, teacherId: req.user._id });
    if (!cls) return res.status(403).json({ message: 'Class not found or access denied' });
...
router.put('/:classId/:eventId', requireAuth, requireTeacher, async (req, res) => {
  try {
    const cls = await Classroom.findOne({ _id: req.params.classId, teacherId: req.user._id });
    if (!cls) return res.status(403).json({ message: 'Class not found or access denied' });
    ...
    if (existing.type !== 'custom') {
      return res.status(400).json({ message: 'Only custom events can be edited directly' });
    }
    ...
    const event = await CalendarEvent.findByIdAndUpdate(req.params.eventId, update, { new: true });
    res.json(event);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.delete('/:classId/:eventId', requireAuth, requireTeacher, async (req, res) => {
  try {
    const cls = await Classroom.findOne({ _id: req.params.classId, teacherId: req.user._id });
    if (!cls) return res.status(403).json({ message: 'Class not found or access denied' });
    ...
    if (!existing) return res.status(404).json({ message: 'Event not found' });
    if (existing.type !== 'custom') {
      return res.status(400).json({ message: 'Only custom events can be deleted directly' });
    }
...
router.get('/:classId/feed.ics', async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="classos-${req.params.classId}.ics"`);
