...
async function assertLessonAccess(req, lesson) {
  if (!lesson?.classId) return { ok: false, status: 404, message: 'Lesson not found' };
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    if (!questionId || !body) {
      return res.status(400).json({ message: 'questionId and body are required' });
    }
    const gate = await assertQuestionAccess(req, questionId);
    if (!gate.ok) return res.status(gate.status).json({ message: gate.message });

    const answer = await Answer.create({ questionId, body, authorId: req.user._id });
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'teacher' && answer.authorId.toString() !== req.user._id.toString()) {
