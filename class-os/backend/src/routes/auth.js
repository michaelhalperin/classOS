...
async function teacherHasStudent(teacherId, studentId) {
  const n = await Classroom.countDocuments({ teacherId, studentIds: studentId });
  return n > 0;
}
...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = await User.create({ name, email, passwordHash, role, lastLoginAt: new Date() });
...
router.get('/students', requireAuth, requireTeacher, async (req, res) => {
  try {
    ...
    const cls = await Classroom.findOne({ _id: classId, teacherId: req.user._id }).populate(
      'studentIds',
      'name email createdAt lastLoginAt role'
    );
    if (!cls) return res.status(404).json({ message: 'Class not found' });
    const list = (cls.studentIds || []).slice().sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    res.json(list);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.post('/students', requireAuth, requireTeacher, async (req, res) => {
  try {
    ...
    const passwordHash = await bcrypt.hash(password, 12);
    const student = await User.create({ name, email, passwordHash, role: 'student' });
    res.status(201).json({
      id: student._id,
      name: student.name,
      email: student.email,
...
router.delete('/students/:id', requireAuth, requireTeacher, async (req, res) => {
  try {
    const ok = await teacherHasStudent(req.user._id, req.params.id);
    if (!ok) return res.status(403).json({ message: 'Not allowed' });
    await Classroom.updateMany({ studentIds: req.params.id }, { $pull: { studentIds: req.params.id } });
...
router.get('/me/completed-lessons', requireAuth, async (req, res) => {
  try {
    ...
    const lessonIds = (user?.completedLessonIds || []).map((id) => id.toString());
    res.json({ lessonIds });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.patch('/me/completed-lessons', requireAuth, async (req, res) => {
  try {
    ...
    if (!lessonId || typeof completed !== 'boolean') {
      return res.status(400).json({ message: 'lessonId and completed (boolean) required' });
    }
    ...
    const user = await User.findByIdAndUpdate(req.user._id, update, { new: true }).select('completedLessonIds');
    const lessonIds = (user?.completedLessonIds || []).map((id) => id.toString());
    res.json({ lessonIds });
  } catch (err) {
    res.status(500).json({ message: err.message });
...
router.post('/me/completed-lessons/sync', requireAuth, async (req, res) => {
  try {
    ...
    if (valid.length === 0) {
      const user = await User.findById(req.user._id).select('completedLessonIds');
      return res.json({ lessonIds: (user?.completedLessonIds || []).map((id) => id.toString()) });
    }
    await User.findByIdAndUpdate(req.user._id, { $addToSet: { completedLessonIds: { $each: valid } } });
    const user = await User.findById(req.user._id).select('completedLessonIds');
...
router.delete('/me', requireAuth, async (req, res) => {
  try {
    ...
    if (user.role === 'student') {
      ...
      await Classroom.updateMany({ studentIds: user._id }, { $pull: { studentIds: user._id } });
