...
router.get("/", requireAuth, async (req, res) => {
  try {
    if (req.user.role === "teacher") {
      ...
      const lessons = await Lesson.find({ classId }).sort({
        weekNumber: 1,
        orderIndex: 1,
      });
    ...
    const lessons = await Lesson.find({ classId: { $in: classIds } }).sort({
      weekNumber: 1,
      orderIndex: 1,
    });
    const out = lessons.map((l) => {
...
router.patch("/reorder", requireAuth, requireTeacher, async (req, res) => {
  try {
    ...
    if (!classId || !Array.isArray(order) || order.length === 0) {
      return res
        .status(400)
        .json({ message: "classId and order[] are required" });
    }
    if (!mongoose.Types.ObjectId.isValid(classId)) {
      return res.status(400).json({ message: "Invalid classId" });
    }
    ...
    const updated = await Lesson.find({ classId }).sort({
      weekNumber: 1,
      orderIndex: 1,
    });
    res.json(updated);
...
router.post("/:id/track-time", requireAuth, async (req, res) => {
  try {
    if (req.user.role !== "student") {
      return res
        .status(403)
        .json({ message: "Only students can track lesson time" });
    }
    const raw = Number(req.body?.seconds);
    const seconds = Number.isFinite(raw)
      ? Math.min(Math.max(0, Math.floor(raw)), 3600)
      : 0;
