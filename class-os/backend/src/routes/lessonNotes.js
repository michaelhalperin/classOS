...
async function assertStudentLessonAccess(studentId, lessonId) {
  ...
  const enrolled = cls.studentIds.some((id) => id.toString() === studentId.toString());
...
router.post('/:lessonId', requireAuth, async (req, res) => {
  try {
    ...
    if (content.length > MAX_CONTENT) {
      return res.status(400).json({ message: `Notes must be at most ${MAX_CONTENT} characters` });
    }
...
router.put('/:lessonId/:noteId', requireAuth, async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(lessonId) || !mongoose.Types.ObjectId.isValid(noteId)) {
