...
import { exampleHourContent, exampleHourObjectives } from './seed/exampleHourLesson.js';
...
async function seed() {
  ...
  const [teacher, s1, s2, s3] = await User.create([
    { name: 'Teacher', email: 'teacher@class.com', passwordHash: await hash('password123'), role: 'teacher' },
    { name: 'Student One', email: 'student1@class.com', passwordHash: await hash('password123'), role: 'student' },
    { name: 'Student Two', email: 'student2@class.com', passwordHash: await hash('password123'), role: 'student' },
    { name: 'Student Three', email: 'student3@class.com', passwordHash: await hash('password123'), role: 'student' },
  ]);

  const mainClass = await Classroom.create({
    name: 'Full Stack 101',
    description: 'Default demo class — lessons, assignments, and roster are scoped here.',
